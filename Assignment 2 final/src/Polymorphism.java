...
public class Publication
{
	...
	public void sellcopy(int qty)
	{
		if(qty<=copies)			//available
			System.out.println("Successfully Sold, Total cost: "+(qty*price));
		else					//unavailable
...
public class Book extends Publication
{
	...
	public void read()
	{
		...
		System.out.println("Enter No of authors: ");	//getting Number of authors
		n=sc.nextInt();
		authors=new String[n];							//initializing string of size n
			
		authors[0]=sc.next();		//skipping '\n'
		for(int i=0;i<n;i++)
		{
	...
	public void sellcopy(int qty)
	{
		if(copies>=qty)					//sufficient Copies available
			System.out.println("Copies sold successfully, Amount Paid: "+(qty*price));
...
public class Polymorphism {
	...
	public static void main(String[] args) {
		...
		Book bk=pb.new Book();					//creating object of Book
		bk.read();
		bk.display();
		System.out.println("Enter the Amount of Copies yout want to Buy: ");
		n=sc.nextInt();				//reading no. of copies to issue
		...
		Magazine mgz=pb.new Magazine();			//creating object of Magazine
		mgz.read();
		mgz.display();
